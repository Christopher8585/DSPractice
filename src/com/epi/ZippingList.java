...
class ZippingList {
	...
	public static ListNode<Integer> zippingLinkedList(ListNode<Integer> L) {
		...
		ListNode<Integer> firstHalfIter = firstHalfHead, secondHalfIter = secondHalfHead;
	...
	public static void main(String[] args) {
		...
		if (args.length > 1) {
			for (String element : args) {
				ListNode<Integer> curr = new ListNode<>(
						Integer.parseInt(element), null);        
				curr.next = head;
				head = curr;
			}
		} else {
		...
		int idx = 0, pre = 0;
			...
			if (args.length <= 1) {
				if ((idx & 1) != 0) {
					System.out.println (pre + curr.data );
				}
			}
      			++idx;
