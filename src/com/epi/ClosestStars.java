...
public class ClosestStars {
	...
	public static List<Star> findClosestKStars(InputStream stars, int k) {
		...
		PriorityQueue<Star> maxHeap = new PriorityQueue<>(k,
				Collections.reverseOrder());
		try {
			ObjectInputStream osin = new ObjectInputStream(stars);

			while (true) {
				// Add each star to the max-heap. If the max-heap size exceeds
				// k,
	...
	public static void main(String[] args) {
		...
		for (int times = 0; times < 1; ++times) {
			...
			for (int i = 0; i < num; ++i) {
				stars.add(new Star(r.nextInt(100001), r.nextInt(100001), r
						.nextInt(100001)));
			}
			...
			//System.out.println (stars.get(k - 1));
		}
	}
